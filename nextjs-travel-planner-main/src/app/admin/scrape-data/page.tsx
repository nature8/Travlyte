...
const ScrapeTrips = () => {
  ...
  //     `https://secure.geonames.org/searchJSON?q=${searchQuery}&maxRows=5&username=prakruti&style=SHORT`
  //   );
  ...
  const searchCities = async (searchQuery: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        setCities([]); // Reset cities if the fetch fails
        return;
      }
  
      const parsed = await response.json();
      setCities(
        parsed?.geonames ? parsed.geonames.map((city: { name: string }) => city.name) : []
      );
    } catch (error) {
      console.error("Failed to fetch city data:", error);
      setCities([]); // Reset cities in case of an error
    }
  };
